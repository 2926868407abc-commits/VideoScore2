...
def main(args):
    ...
    if method.lower() == "vs2_int":
        ## conda activate vs2_eval
        from eval_methods.vs2_int import eval_VideoScore2_int
        model=eval_VideoScore2_int(model_name_or_path)    
        q_template=VS2_QUERY_TEMPLATE
    
    # default float score: expected score over the legal score space {1,2,3,4,5}
    elif method.lower() in ["vs2_float","vs2_float_normed"]:
        ## conda activate vs2_eval
        from eval_methods.vs2_float import eval_VideoScore2_float
        model=eval_VideoScore2_float(model_name_or_path,floating_method="expected")    
        q_template=VS2_QUERY_TEMPLATE
    
    elif method.lower() == "vs2_float_weighted":
        ## conda activate vs2_eval
        from eval_methods.vs2_float import eval_VideoScore2_float
        model=eval_VideoScore2_float(model_name_or_path,floating_method="confidence_adjusted")    
        q_template=VS2_QUERY_TEMPLATE
    ...
    if method in ["vs2_int","vs2_float","vs2_float_normed","vs2_float_weighted",]:
    ...
    # keep legacy file naming for compatibility
    if method in ["vs2_float","vs2_float_normed",]:
        temperature=kwargs.get("temperature",0.7)
        eval_res_path=eval_res_path.replace(".json",f"_float_normed_tempe={temperature}.json")
    
    # legacy confidence-adjusted float score
    if method == "vs2_float_weighted":
        temperature=kwargs.get("temperature",0.7)
        eval_res_path=eval_res_path.replace(".json",f"_float_weighted_tempe={temperature}.json")
    ...
    for item in tqdm(bench_data):
        ...
        try:
            ...
            s_t=time.time()
            extra_scores = None
            if hasattr(model, "evaluate_video_structured") and method.lower() in ["vs2_float", "vs2_float_normed", "vs2_float_weighted"]:
                extra_scores, raw_output = model.evaluate_video_structured(user_prompt, path_or_url, kwargs)
                if method.lower() in ["vs2_float", "vs2_float_normed"]:
                    v_out = extra_scores["v_expected_score"]
                    t_out = extra_scores["t_expected_score"]
                    p_out = extra_scores["p_expected_score"]
                else:
                    v_out = extra_scores["v_confidence_adjusted_score"]
                    t_out = extra_scores["t_confidence_adjusted_score"]
                    p_out = extra_scores["p_confidence_adjusted_score"]
            else:
